...
def train(text,gram,mysetofchars):
##  Clean the text
  text='_'.join(text.split()) 
  text=text.lower() 
  setofchars=''.join(set(text)) 

  num_chars=len(mysetofchars)

  for char in setofchars:
      if char not in mysetofchars:
...
## Train the model
  ...
  markovtrain=np.zeros(shape=(num_chars,num_chars*num_chars),dtype=float)
